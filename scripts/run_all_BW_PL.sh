#!/bin/bash
# Master script to run all BW+PL (Best-Worst + Plackett-Luce) experiments across environments and model types
# Total: 3 envs × 2 model types = 6 experiments

ENVIRONMENTS=("box-close-v2" "button-press-topdown-v2" "drawer-open-v2")
MODEL_TYPES=("PL" "linear_PL")
METHOD="BW_PL"
...
echo "========================================"
echo "BW+PL (Best-Worst + Plackett-Luce) EXPERIMENTS BATCH"
...
for env in "${ENVIRONMENTS[@]}"; do
    for model in "${MODEL_TYPES[@]}"; do
        ...
        # Run the experiment
        bash scripts/MetaWorld_BW_PL.sh
...
echo ""
echo "========================================"
echo "ALL BW+PL EXPERIMENTS COMPLETED"
